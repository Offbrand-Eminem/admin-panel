...
public class DeopCommand {
    ...
    private static int removeAdmin(CommandContext<ServerCommandSource> context) {
        ...
        if (target != null) {
            target.sendMessage(Text.literal("§cYou have been demoted from admin"));
        }
        
        return 1;
    }
    
    private static int removeOwner(CommandContext<ServerCommandSource> context) {
        String playerName = StringArgumentType.getString(context, "player");
        ServerCommandSource source = context.getSource();
        ...
        ServerPlayerEntity target = source.getServer().getPlayerManager().getPlayer(playerName);
        if (target != null) {
            target.sendMessage(Text.literal("§cYou have been demoted from owner"));
        }
