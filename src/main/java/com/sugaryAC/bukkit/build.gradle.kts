...
bukkit {
    ...
    permissions {
        register("sugaryac.alerts") {
            description = "Receive alerts for violations"
            default = Permission.Default.OP
        }

        register("sugaryac.alerts.enable-on-join") {
            description = "Enable alerts on join"
            default = Permission.Default.OP
        }

        register("sugaryac.performance") {
            description = "Check performance metrics"
            default = Permission.Default.OP
        }

        register("sugaryac.profile") {
            description = "Check user profile"
            default = Permission.Default.OP
        }

        register("sugaryac.brand") {
            description = "Show client brands on join"
            default = Permission.Default.OP
        }

        register("sugaryac.brand.enable-on-join") {
            description = "Enable showing client brands on join"
            default = Permission.Default.OP
        }

        register("sugaryac.sendalert") {
            description = "Send cheater alert"
            default = Permission.Default.OP
        }

        register("sugaryac.nosetback") {
            description = "Disable setback"
            default = Permission.Default.FALSE
        }

        register("sugaryac.nomodifypacket") {
            description = "Disable modifying packets"
            default = Permission.Default.FALSE
        }

        register("sugaryac.disabled") {
            description = "Disable sugaryAC checks while keeping player state tracked"
            default = Permission.Default.FALSE
        }

        register("sugaryac.exempt") {
            description = "Exempt from all checks"
            default = Permission.Default.FALSE
        }

        register("sugaryac.verbose") {
            description = "Receive verbose alerts for violations"
            default = Permission.Default.OP
        }

        register("sugaryac.verbose.enable-on-join") {
            description =
                "Enable verbose alerts on join"
            default = Permission.Default.FALSE
        }

        register("sugaryac.list") {
            description =
