...
public class GrimExternalAPI implements GrimAbstractAPI, ConfigReloadObserver, StartableInitable {
    ...
    private static String resolveGrimVersion(GrimAPI api) {
        try {
            Properties properties = PropertiesUtil.readProperties(GrimExternalAPI.class, "sugaryac.properties");
    ...
    public void onReload(ConfigManager newConfig) {
        ...
        // First-load guard: load() calls reload() before start() runs, so this fires once with started=false before the datastore exists. Subsequent /sugaryac reload calls see started=true and proceed (including disabled→enabled flips — DataStoreLifecycle.reload() re-evaluates builder.enabled() each time).
        if (!started) return;
