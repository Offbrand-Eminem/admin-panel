...
public class CloudCommandService implements CommandService {
    ...
    private void registerInvalidSyntaxHandler(CommandManager<Sender> commandManager) {
        commandManager.exceptionController().registerHandler(InvalidSyntaxException.class, context -> {
            ...
            if (isHistoryInput(context.context().rawInput().input())) {
                ...
                sender.sendMessage(Component.text("Use: /sugaryac history <player> [page <N>]", NamedTextColor.GRAY));
                sender.sendMessage(Component.text("Use: /sugaryac history <player> session <N|latest> [page <N>] [-d] [-v]", NamedTextColor.GRAY));
                sender.sendMessage(Component.text("Tip: /sugaryac history <player> session shows filter and detail options.", NamedTextColor.GRAY));
                sender.sendMessage(Component.text("Use /sugaryac history player <player> ... for names that collide with history subcommands.", NamedTextColor.GRAY));
                return;
            }
    ...
    private static boolean isHistoryInput(String rawInput) {
        ...
        return tokens.length >= 2
                && (tokens[0].equals("sugaryac") || tokens[0].equals("sugaryAC"))
